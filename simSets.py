...
with open("submissions.csv",'r') as f:
	with open("kappa_scores.csv",'w') as w:
		with open("final_orderings.csv",'w') as p:
			pwriter = csv.writer(p)

			sponsReader = list(csv.reader(f))
			pwriter.writerow(['attribute','common dependencies'])
			writer = csv.writer(w)
			...
			writer.writerow(['Attribute','Interpretation','Kappa','Average # added','Average # removed','shared_adjectives','shared_dependencies'])
			...
			for attr_tup in attr_list:
				...
				try:
					max_fleiss = fleiss_kappa(mat_li,5,3)
				except:
					max_fleiss = 0

				for dep in dep_list:
					shared = 3
					tups = [tup for tup in mat_li if tup[0] == dep]
					for x in [-1,0,1]:
						if len([tup for tup in tups if tup[1] == x]) >= 3:
							shared = x	
					'''
					print tups
					tup_mat = [[]]
					tup_mat[0].append(len([tup for tup in tups if tup[1] == -1]))
					tup_mat[0].append(len([tup for tup in tups if tup[1] == 0]))
					tup_mat[0].append(len([tup for tup in tups if tup[1] == 1]))
					try:
						#dep_fleiss = fleiss_kappa(tups,5,3)
						dep_fleiss = computeKappa(tup_mat)
						print dep_fleiss
					except:
						dep_fleiss = 0
						print dep_fleiss
				...
				writer.writerow([attr_tup[0],interpretation,max_fleiss,sum(added)/float(len(added)),sum(removed)/float(len(removed)),list(result)] + comp_list + shared_deps)
				pwriter.writerow([attr_tup[0]] + shared_deps)
				added = []
				removed = []
